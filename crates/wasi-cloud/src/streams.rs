...
impl InputStream {
    pub async fn read(
        ...
        let len = usize::try_from(len).map_err(|_| streams::StreamError::Error)?;
                ...
                let ended = if let Some(notify) = notify {
                    ...
                    match Pin::new(&mut self.body.trailers()).poll(&mut Context::from_waker(
                        ...
                        Poll::Ready(trailers) => {
                            ...
                            self.trailers = trailers.map_err(|_| streams::StreamError::Error)?;
                            true
                        }
                    }
                } else {
                    self.trailers = self
                        ...
                        .trailers()
                        .await
                        .map_err(|_| streams::StreamError::Error)?;
                    true
                };
                    ...
                    match Pin::new(&mut self.body).poll_next(&mut Context::from_waker(
                        ...
                        Poll::Ready(chunk) => {
                            ...
                            if let Some(chunk) = chunk {
                                let chunk = chunk.map_err(|_| streams::StreamError::Error)?;
...
impl OutputStream {
    pub async fn write(
        ...
        if let Sender::Hyper(None) = &self.sender {
            return Err(streams::StreamError::Error);
        }
            ...
            match poll {
                ...
                Poll::Ready(result) => {
                    ...
                    match result {
                        ...
                        Err(()) => Err(streams::StreamError::Error),
                    }
                }
            }
        } else {
